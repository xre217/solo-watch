...
export function formatMarkdown(r: ScanReport): string {
  const problems = r.findings.filter((f) => f.severity !== "ok");
  const oks = r.findings.filter((f) => f.severity === "ok");
  const lines = [
    `# solo-watch`,
...
export function formatAnnotations(r: ScanReport): string {
  const lines: string[] = [];
  for (const f of r.findings) {
    if (f.severity === "ok") continue;
    const level =
      f.severity === "fail" ? "error" : f.severity === "warn" ? "warning" : "notice";
  ...
  lines.push(
    `::notice title=solo-watch score::grade ${r.grade} score ${r.score}/100`,
  );
  return lines.join("\n");
}
