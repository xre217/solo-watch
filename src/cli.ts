...
async function main(): Promise<void> {
  ...
  const { json, minScore, history, badge, delta, limit, interval, positional } =
    opts;
  ...
  if (head === "history") {
    ...
    if (json) console.log(JSON.stringify(lines, null, 2));
      ...
      for (const l of lines) {
        console.log(
          `${l.ts.slice(0, 19)}  ${l.grade} ${String(l.score).padStart(3)}  ${l.finding_ids.join(",") || "—"}`,
        );
      }
    }
    return;
  ...
  process.exit(
    runOnce(path.resolve(target), { json, minScore, history, badge, delta }),
  );
}
